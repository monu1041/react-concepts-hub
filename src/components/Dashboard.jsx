...
export default function Dashboard({ topic }) {
  ...
  // Separate loading states for smooth Concurrent UI transitions
  ...
  // 2️⃣ Load README string when user clicks "📖 README"
  useEffect(() => {
    ...
    async function fetchReadme() {
      try {
        ...
        if (isMounted) {
          setReadmeContent(text || ''); // Empty string flags that we already attempted fetch
        }
      } catch (err) {
  ...
  // 3️⃣ Load Source Code strings when user clicks "💻 Source Code"
  useEffect(() => {
  ...
  return (
    ...
    >
        ...
        {activeTab === 'ui' && (
          isUiLoading || !Component ? (
            ...
            <div 
              ...
              style={{ 
                all: 'initial', display: 'block', fontFamily: 'sans-serif', width: '100%',
                padding: '10px', borderRadius: '4px', color: '#0f172a', backgroundColor: '#ffffff',    
      ...
      <style>{`
        ...
        .live-ui-sandbox > *:first-child { margin-top: 0 !important; padding-top: 0 !important; }
        @media (max-width: 768px) { .dashboard-main-panel { height: auto; overflow-y: visible; padding: 12px 16px 24px 16px !important; } }
      `}</style>
    </main>
  );
}